...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Always try user auth if admin auth fails
      // Don't return early - let it fall through to user auth attempt
      ...
      try {
        ...
        const isMaster = user.isMaster === true || user.role === 'admin';

        // Master users don't need tenant assignment
        if (userTenants.length === 0 && !isMaster) {
          return NextResponse.json(
